...
| C-09 | "Prevent accidental exit from an activity" (§13) vs "easy home button" (§13). | §13 | Home button always visible; leaving mid-activity uses a child-operable two-step ("hold to go home" 1.2 s press) not a parental gate; work auto-saves first (§22). |
