...
- **Components:** `BigTile`, `ActivityCard`, `InstructionBar` (replay ▶ always visible), `HoldToHomeButton`, `StarBurst`, `DemoHand`, `CanvasToolbar` (mirrors for left-handers), `ParentListRow`.
...
| S10 | Drawing board | Child | Full-bleed Skia canvas; toolbar (mirrorable): brush type (crayon/pencil/marker/paint), size (3), colour wells + pastel page + glitter + rainbow, sticker tray, eraser, undo/redo, clear (two-step), save; hold-to-home; replay-my-drawing button in gallery |
