# Little Grip - children's fine-motor-skills learning app
...
> **Status:** Full-scope build in progress (owner direction 16 Jul 2026 — see docs/01 scope revision). Domain engines are implemented and fully tested; the Expo app is a
...
| `packages/core` | `@littlegrip/core` - pure TypeScript domain logic: tracing corridor engine, puzzle snap/hints, rewards (daily-capped), screen-time ledger, parental gate, recommendations, daily plan, progress/report, content-pack schema, deletion. **96 unit tests.** |
...
All artwork lives in **`assets/images/`** (original SVGs generated by `assets/generate-images.mjs`
— no third-party art). Replace any `.svg` (same filename) and rebuild to swap it everywhere:
puzzles, matching/memory, shadow games, feeding, mazes, reveal-and-wipe, counting and stickers.
See `assets/images/README.md`. Instruction voice is a **soft female voice** (browser speech
synthesis in the demo; recorded voice in production per docs/01 A-12).
